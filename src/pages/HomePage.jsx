...
<div style={{ 
  ...
  pointerEvents: 'auto' // নিশ্চিত করে যে মাউস ক্লিক/হুইল কাজ করবে
      ...
      {/* App Download Banner */}
      {isMobile && (
        <div style={{ margin: '20px', background: 'linear-gradient(135deg, #005a38 0%, #00331e 100%)', borderRadius: '24px', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 10px 25px rgba(0, 51, 30, 0.25)', border: '1px solid rgba(255, 255, 255, 0.1)', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', right: '-10px', bottom: '-20px', opacity: 0.08, color: 'white', transform: 'rotate(-20deg)' }}>
            <TrainFront size={90} />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, paddingRight: '15px', zIndex: 1 }}>
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          {displayTrains.map((group, idx) => (
            <div key={idx} onClick={() => {setSelectedTrainGroup(group); setActiveDirectionIndex(0); sessionStorage.setItem('lastSelectedTrain', JSON.stringify(group));}} 
                 style={{ backgroundColor: 'white', borderRadius: '25px', padding: '16px', display: 'flex', alignItems: 'center', gap: '15px', boxShadow: '0 4px 15px rgba(0,0,0,0.04)', borderLeft: '6px solid #006a4e', border: '1px solid #f0f0f0', cursor: 'pointer' }}>
              <div style={{ width: '70px', height: '70px', borderRadius: '15px', overflow: 'hidden', backgroundColor: '#f8f9fa', flexShrink: 0 }}>
                <img src="/homeimg.png" alt={group[0].name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
              </div>
              <div style={{ flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <span style={{ color: '#006a4e', fontSize: '10px', fontWeight: 'bold' }}>কোড: {group[0].id}</span>
                  <h4 style={{ margin: '2px 0', fontSize: '16px', color: '#1e293b', fontWeight: '800' }}>{group[0].name}</h4>
                  <span style={{ fontSize: '11px', color: '#94a3b8' }}>ছুটি: {group[0].offDay}</span>
                </div>
                <button onClick={(e) => {e.stopPropagation(); setSelectedTrainGroup(group); setActiveDirectionIndex(0); sessionStorage.setItem('lastSelectedTrain', JSON.stringify(group));}} 
                        style={{ backgroundColor: '#006a4e', color: 'white', border: 'none', padding: '10px 12px', borderRadius: '12px', fontSize: '11px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <Activity size={14} /> Tracking
                </button>
              </div>
            </div>
      ...
      <div style={{ padding: '0 20px 20px' }}>
        <div onClick={() => navigate('/metro-rail')} style={{ background: 'linear-gradient(135deg, #008352 0%, #005a38 100%)', borderRadius: '30px', padding: '25px', color: 'white', boxShadow: '0 15px 30px rgba(0,131,82,0.2)', position: 'relative', overflow: 'hidden', cursor: 'pointer' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <h2 style={{ margin: '0 0 5px 0', fontSize: '24px', fontWeight: '900' }}>বাংলাদেশ মেট্রো রেল</h2>
            <p style={{ margin: '0 0 20px 0', fontSize: '13px', opacity: 0.8, lineHeight: '1.5', maxWidth: '80%' }}>সময়সূচী, ভাড়া, টিকিট ও যাতায়াতের সকল তথ্য এখন এক জায়গায়।</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
              {[{ icon: <Clock size={14} />, text: 'সময়সূচী' }, { icon: <CreditCard size={14} />, text: 'টিকিট কার্ড' }, { icon: <Map size={14} />, text: 'রুট ম্যাপ' }, { icon: <Gavel size={14} />, text: 'আইন-কানুন' }].map((item, i) => (
