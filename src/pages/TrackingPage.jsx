...
const TrackingPage = () => {
  ...
  useEffect(() => {
    ...
    if (isSharing && isTraveler && train && scheduleData.length > 0) {
      watchId = navigator.geolocation.watchPosition((pos) => {
        ...
        // অ্যাপের জিপিএস স্পীড ফিল্টারিং
        let rawSpeed = speed;
        if (rawSpeed === null || rawSpeed === undefined || rawSpeed < 0) {
          rawSpeed = 0;
        }
        const currentSpeed = Math.round(rawSpeed * 3.6);
        const isMoving = currentSpeed >= 5;

        // রেললাইন থেকে দূরত্ব চেক করার লজিক (২০ মিটার = ০.০২ কিমি)
        const [snappedLat, snappedLng] = getSnappedPoint(latitude, longitude, railData);
        const distanceToRailLine = calculateDistance(latitude, longitude, snappedLat, snappedLng);
        const isNearRailLine = distanceToRailLine <= 0.02;
        ...
        // মেইন কঠোর শর্ত ফিল্টার
        if (isMoving && isNearRailLine && isWithinTime) {
          // --- সঠিক স্টেশন ইনডেক্স ক্যালকুলেশন ---
          let matchedIdx = 0; 
          let minD = Infinity;
          scheduleData.forEach((st) => {
  ...
  return (
    <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ padding: '20px 20px 50px' }}>
        <div style={{ background: 'white', borderRadius: '24px', padding: '25px', boxShadow: '0 5px 15px rgba(0,0,0,0.05)' }}>
          <h4 style={{ marginBottom: '25px', fontWeight: 'bold', color: '#333' }}>ট্রেনটি নিচের অবদান স্টেশনগুলোতে থামবে</h4>
          {train.stations.map((st, i) => {
            const isPassed = activeState.isRunning ? (i < activeState.index) : false;
            const isCurrent = activeState.isRunning ? (i === activeState.index) : false;
            const schedMin = parseToMinutes(st.arrival === 'START' ? st.departure : st.arrival);
