...
//           // --- সঠিক স্টেশন ইনডেক্স ক্যালকুলেশন ---
//           let matchedIdx = 0; 
//           let minD = Infinity;
...
//             lng: longitude, 
//             speed: currentSpeed,
//             index: matchedIdx // সার্ভারে ইনডেক্স পাঠানো হচ্ছে
//           };
...
//       // লাইভ ডাটার ইনডেক্স যদি ডাটাবেস থেকে আসে তবে সেটি ব্যবহার করা, নাহলে ক্যালকুলেট করা
//       let matchedIdx = liveData.index !== undefined ? liveData.index : 0;
//       if (liveData.index === undefined) {
//         let minD = Infinity;
//         scheduleData.forEach((st) => {
//           const coords = stationCoords[st.name.trim()];
//           if (coords) {
...
import { MapContainer, TileLayer, Marker, Tooltip, useMap, GeoJSON } from 'react-leaflet';
import L from 'leaflet';
import { ChevronLeft, AlertCircle, Clock, Navigation } from 'lucide-react';
import { io } from 'socket.io-client';
import { trains } from '../data/trainData';
import { stationCoords } from '../data/stationCoords';
...
const TrackingPage = () => {
  ...
  // =========================================================================
  // অটো-সিঙ্ক ফেচ লুপ (প্রতি ২৫ সেকেন্ডে স্বয়ংক্রিয় ব্যাকগ্রাউন্ড আপডেট)
  // =========================================================================
  useEffect(() => {
    if (!trainId) return;
    ...
    const syncInterval = setInterval(fetchLiveStatus, 25000);
  ...
  const activeState = useMemo(() => {
    ...
    const getScheduledState = () => {
      ...
      if (inSchedule) {
        ...
        if (!lat) {
          ...
          currentIndex = scheduleData[lastStIdx].idx;
          const coords = stationCoords[train.stations[currentIndex].name.trim()];
          lat = coords ? coords[0] : 23.81; 
          lng = coords ? coords[1] : 90.41;
        }
        const [sLat, sLng] = getSnappedPoint(lat, lng, railData);
        return { lat: sLat, lng: sLng, speed: 45, mode: 'SCHEDULED', isRunning: true, index: currentIndex, delay: 0 };
      }
    ...
    // =========================================================================
    // ৩ স্তরের ট্র্যাকিং লজিক (১০ মিনিট = LIVE, ১০-১২০ মিনিট = PREDICTED, >১২০ = SCHEDULED)
    // =========================================================================
    if (liveData && liveData.source !== 'SCHEDULED' && liveData.mode !== 'SCHEDULED' && (liveData.lat || liveData.lastLocation?.lat || liveData.source === 'EXTERNAL')) {
      let matchedIdx = liveData.index !== undefined ? liveData.index : (liveData.currentStationIndex || 0);
      let rawLat = Number(liveData.lat || liveData.lastLocation?.lat);
      let rawLng = Number(liveData.lng || liveData.lastLocation?.lng);

      const delay = liveData.delay || 0;
      let baseSpeed = liveData.speed || 0;
      let currentSpeed = baseSpeed;

      const currentSt = train.stations[matchedIdx] || train.stations[0];
      const nextSt = train.stations[matchedIdx + 1] || currentSt;
      const currentCoords = stationCoords[currentSt.name.trim()] || [23.81, 90.41];
      const nextCoords = stationCoords[nextSt.name.trim()] || currentCoords;

      let lat = rawLat;
      let lng = rawLng;

      // থার্ড-পার্টি ডেটা হলে (বা সরাসরি Lat/Lng না থাকলে) শিডিউল ও বিলম্ব মিলিয়ে প্রগ্রেস ইন্টারপোলেশন
      if (liveData.source === 'EXTERNAL' || isNaN(rawLat) || isNaN(rawLng)) {
        const depTimeMin = parseToMinutes(currentSt.departure === "END" ? currentSt.arrival : currentSt.departure);
        const arrNextMin = parseToMinutes(nextSt.arrival === "START" ? nextSt.departure : nextSt.arrival);

        let progress = 0;

        if (depTimeMin !== null && arrNextMin !== null && nextSt !== currentSt) {
          const actualDepTime = depTimeMin + delay;
          let actualArrNext = arrNextMin + delay;

          if (actualArrNext < actualDepTime) actualArrNext += 1440;

          let currentNowMin = currentMinWithSec;
          if (currentNowMin < actualDepTime && actualArrNext > 1440) currentNowMin += 1440;

          const totalSegmentMinutes = actualArrNext - actualDepTime;
          const elapsedMinutes = currentNowMin - actualDepTime;

          if (totalSegmentMinutes > 0) {
            progress = Math.min(Math.max(elapsedMinutes / totalSegmentMinutes, 0.05), 0.95);
          }
        }

        lat = currentCoords[0] + (nextCoords[0] - currentCoords[0]) * progress;
        lng = currentCoords[1] + (nextCoords[1] - currentCoords[1]) * progress;

        if (currentSpeed === 0) currentSpeed = 45;
      } else {
        // লোকাল ইউজারের লাইভ জিপিএস থাকলে গতি ও সময়ের ব্যবধানে ট্র্যাকিং মুভমেন্ট
        const updateTime = liveData.updatedAt || liveData.lastLocation?.updatedAt;
        const lastUpdateTs = updateTime ? new Date(updateTime).getTime() : currentTime;
        const diffInSec = Math.max(0, (currentTime - lastUpdateTs) / 1000);
        const distanceMoved = (currentSpeed / 3600) * diffInSec;

        if (nextCoords && distanceMoved > 0) {
          const totalDistToNext = calculateDistance(rawLat, rawLng, nextCoords[0], nextCoords[1]);
          if (totalDistToNext > 0) {
            const ratio = Math.min(distanceMoved / totalDistToNext, 0.999); 
            lat = rawLat + (nextCoords[0] - rawLat) * ratio;
            lng = rawLng + (nextCoords[1] - rawLng) * ratio;
          }
      ...
      const updateTime = liveData.updatedAt || liveData.lastLocation?.updatedAt;
      const lastUpdateTs = updateTime ? new Date(updateTime).getTime() : currentTime;
      const diffInMin = Math.max(0, (currentTime - lastUpdateTs) / (1000 * 60)); 
      ...
      if (diffInMin <= 10) {
        ...
        mode = 'PREDICTED';
        if (diffInMin > 30) {
          const reductionProgress = Math.min(1, (diffInMin - 30) / 60); 
      ...
      // রেললাইনের সাথে ট্রেনের পয়েন্ট স্ন্যাপ করা
      const [finalLat, finalLng] = getSnappedPoint(lat, lng, railData);
      return { 
        lat: finalLat, 
        lng: finalLng, 
    ...
    // ৩য় লেভেল: কোনো লাইভ ডাটা না থাকলে শিডিউল মোডে চলবে
    return getScheduledState();
  }, [train, liveData, currentTime, scheduleData]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      distNext: dNext.toFixed(2), 
      distDest: dDest.toFixed(2),
      etaNext: minutesToTime(nextArrivalMin !== null ? nextArrivalMin + activeState.delay : null),
      etaDest: minutesToTime(parseToMinutes(lastSt.arrival) !== null ? parseToMinutes(lastSt.arrival) + activeState.delay : null)
    };
  }, [activeState, train]);
        ...
        <div style={{ 
          ...
          {!activeState.isRunning ? (
            ...
            <>
              {activeState.mode === 'LIVE' && (
                <div style={{ display: 'flex', gap: '8px' }}>
                  <span>{liveData?.source === 'EXTERNAL' ? '📡' : '🛰️'}</span>
                  <span>
                    {liveData?.source === 'EXTERNAL' 
                      ? <>ট্রেনটির <b>লাইভ স্যাটেলাইট ট্র্যাকিং</b> সচল রয়েছে।</> 
                      : <>এই ট্রেনটির <b>লাইভ লোকেশন</b> শেয়ার করা হয়েছে।</>}
                  </span>
                </div>
              )}
              {activeState.mode === 'PREDICTED' && (
                <div style={{ display: 'flex', gap: '8px' }}>
                  <span>⚠️</span>
                  <span>এই ট্রেনটির লাইভ লোকেশন কিছুক্ষণ আগে আপডেট করা হয়েছিল, তাই <b>প্রেডিকশন</b> করে সম্ভাব্য অবস্থান দেখানো হচ্ছে।</span>
                </div>
              )}
              {activeState.mode === 'SCHEDULED' && (
                <div style={{ display: 'flex', gap: '8px' }}>
                  <span>📅</span>
                  <span>এই ট্রেনটির লাইভ লোকেশন পাওয়া যায়নি, তাই <b>শিডিউল</b> অনুযায়ী সম্ভাব্য অবস্থান দেখানো হচ্ছে।</span>
                </div>
              )}
            </>
          )}
      ...
      <div style={{ padding: '20px 20px 50px' }}>
        <div style={{ background: 'white', borderRadius: '24px', padding: '25px', boxShadow: '0 5px 15px rgba(0,0,0,0.05)' }}>
          ...
          {train.stations.map((st, i) => {
            ...
            let stayTime = 0;
            if (st.arrival !== "START" && st.departure !== "END") {
                const arrMin = parseToMinutes(st.arrival);
                const depMin = parseToMinutes(st.departure);
                if (arrMin !== null && depMin !== null) {
                  stayTime = depMin - arrMin;
                  if (stayTime < 0) stayTime += 1440;
                }
            }
            return (
              <div key={i} style={{ display: 'flex', gap: '20px', opacity: isPassed ? 0.6 : 1 }}>
